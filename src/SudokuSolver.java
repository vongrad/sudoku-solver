...
public class SudokuSolver implements ISudokuSolver {
	...
	public void setup(int size1) {
		...
		size = size1;
		int numberOfVars = size*size*size*size;

		puzzle = new int[size*size][size*size];

		D = new ArrayList<>(numberOfVars);
	...
	public ArrayList<Integer> FC(ArrayList<Integer> asn) {
		...
		// If the assignment does not contains any 0, it is already solved
		if(!asn.contains(0)) {
			return asn;
		}

		// Get the first unassigned variable
		Integer X = asn.indexOf(0);

		// Make copy of the domains
		ArrayList<ArrayList<Integer>> _D = deepCopy(D);

		// Iterate through every value of Dx
		for (Integer V: _D.get(X)) {
			// Check if the assignment is consistent after assigning value V
			if(AC_FC(X, V)) {
				asn.set(X, V);

				// Recursively repeat previous steps for all other unassigned variables
				ArrayList<Integer> R = FC(asn);
				...
				// Reset the variable assignment
				asn.set(X, 0);

				// Roll-back the domains
				D = deepCopy(_D);
			}
			else {
				// Roll-back the domains
	 ...
	 * @return copied array
	 */
	public ArrayList<ArrayList<Integer>> deepCopy(ArrayList<ArrayList<Integer>> arr) {
		ArrayList<ArrayList<Integer>> _arr = new ArrayList<>();

		for (ArrayList<Integer> subArr: arr) {
			ArrayList<Integer> _subArr = new ArrayList<>();
